document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchBookings() {
        try {
            const response = await fetch('https://ehs-deploy-8yty.vercel.app/api/bookings');
            if (!response.ok) {
    ...
    bookingsTableBody.addEventListener('click', async (event) => {
        ...
        if (event.target.closest('.edit-button')) {
            ...
            if (confirm('Are you sure you want to delete this booking?')) {
                try {
                    const response = await fetch(`https://ehs-deploy-8yty.vercel.app/api/bookings/${bookingId}`, {
                        method: 'DELETE',
                    });
                    if (!response.ok) {
    ...
    bookingsTableBody.addEventListener('change', async (event) => {
        if (event.target.classList.contains('status-dropdown')) {
            ...
            try {
                const response = await fetch(`https://ehs-deploy-8yty.vercel.app/api/bookings/${bookingId}/status`, {
                    method: 'PUT',
    ...
    editBookingForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (newDate && newTimeSlot) {
                const rescheduleResponse = await fetch(`https://ehs-deploy-8yty.vercel.app/api/bookings/${bookingId}/reschedule`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
