document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchPayments() {
        try {
            const response = await fetch('https://ehs-deploy-sooty.vercel.app/api/payments');
    ...
    paymentsTableBody.addEventListener('change', async (event) => {
        if (event.target.classList.contains('status-dropdown')) {
            ...
            if (!paymentId) return;
            try {
                const response = await fetch(`https://ehs-deploy-sooty.vercel.app/api/payments/${paymentId}`, {
                    method: 'PUT',
